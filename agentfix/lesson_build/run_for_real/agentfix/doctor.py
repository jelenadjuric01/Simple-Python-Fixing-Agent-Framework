...
def _memory_bytes() -> tuple[int | None, int | None]:
    ...
    if sys.platform == "win32":
        # No /proc and no sysctl here, so this one goes through the Win32 API directly.
        # GlobalMemoryStatusEx is the right call for three reasons: ctypes puts it in the
        # standard library, it runs in-process rather than costing a subprocess, and it fills in
        # both numbers this function promises at once. The alternatives are worse: `wmic` has
        # been deprecated since Windows 10 21H1 and is absent from current Windows 11 images, so
        # it would work on the machine you tested and fail on a learner's, and PowerShell's
        # Get-CimInstance needs a second query in different units to get free memory.
        #
        # ctypes is imported here rather than at the top of the file because this is the only
        # platform that needs it.
        import ctypes

        class MemoryStatusEx(ctypes.Structure):
            """The Win32 MEMORYSTATUSEX struct. Field order and widths are the ABI — the API
            reads the struct by offset, so reordering these silently returns nonsense."""
        ...
        try:
            ...
            # The API requires being told how big the struct is before it will fill it in.
...
def _check_ram() -> Check:
    ...
    if total < MIN_TOTAL_RAM_BYTES:
        return Check(
            "ram",
            False,
            f"{detail} — under 16 GB, run `python setup.py --tier qwen` for the small model, "
            "or use the Colab notebook; see README.md",
        )
...
"""
    ...
    return Check(
        "sandbox",
        result.passed,
        "executes tests" if result.passed else result.output,
    )
