"""The command line: `agentlang doctor`, `agentlang solve`, `agentlang eval`.

Registered as a console script in pyproject.toml, which is what makes `uv run agentlang ...`
...
branch. That keeps `agentlang --version` from importing LangGraph or the eval harness, and it
...
from agentlang import __version__
from agentlang.agent.graph import MAX_STEPS


def build_parser() -> argparse.ArgumentParser:
    """Declare the CLI. Separate from `main` so tests can inspect it without running anything."""
    parser = argparse.ArgumentParser(prog="agentlang", description="A teaching coding agent")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.version:
        print(f"agentlang {__version__}")
        return 0

    if args.command == "doctor":
        from agentlang.doctor import report, run_checks
    ...
    if args.command == "solve":
        ...
        # Exit code follows the verdict, so `agentlang solve ... && echo ok` behaves sensibly and
        # CI can gate on it.
        return 0 if result.solved else 1

    if args.command == "eval":
        from agentlang.eval.runner import run_suite
