...
write directly on the host. The reason is that a sandbox exists to contain *code execution*,
and writing bytes to a .py file executes nothing — those bytes stay inert until unittest
imports them. `run_tests` is the moment model-authored code actually runs, so that is where
the boundary goes.
...
class ExecutionBackend(Protocol):
    """One method: run this command in this directory.

    `Protocol` means *structural* typing: a class satisfies this by having a matching method,
    not by inheriting from it. Two implementations satisfy it without naming it —
    `SubprocessBackend` and `DockerBackend` — which is why the tests that assert on the Docker
    command line can run with no Docker daemon present.
    """
...
def get_backend(name: str | None = None) -> ExecutionBackend:
    ...
    subprocess (default) — hardened, not isolated. Same machine, same user, same filesystem.
        Adds a stripped environment, resource limits and a timeout. Test code can still open
        sockets and read anything you can read.
    docker (opt-in)      — real isolation: no network, read-only mount, dropped capabilities,
        memory and pid caps. Needs an image built first.

    The imports are inside the branches so that importing this module never requires Docker.
    """
    ...
    if choice == "docker":
        from agentlang.sandbox.docker_backend import DockerBackend
