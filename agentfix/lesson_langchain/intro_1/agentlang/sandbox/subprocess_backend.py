...
It runs the tests in a separate process with a stripped environment, resource limits and a
timeout, which stops runaway resource use and accidental damage. It does NOT isolate the
filesystem or the network: test code runs as your user, on your machine.
...
# without interfering with a normal unittest run of a few small files.
...
def _apply_limits() -> None:
    ...
    # RLIMIT_AS is Linux-only: on macOS/Apple Silicon it aborts interpreter startup entirely
    # (CPython reserves a large virtual address space up front), so it is only safe to apply
    # on Linux — which is also where the memory-constrained student environments run.
...
class SubprocessBackend:
    ...
    def run(self, workspace: Path, command: tuple[str, ...], timeout_s: int = 10) -> ExecResult:
        ...
        try:
            completed = subprocess.run(
                list(command),
                # Run from inside the workspace, so the task's own imports resolve against the
                # copy rather than the repo. unittest also puts this directory on sys.path.
                cwd=str(workspace),
                env=env,
                capture_output=True,
                text=True,
        ...
        # stdout and stderr merged because unittest writes its report to STDERR while a task's
        # own print() goes to stdout, and the model needs to read them as one narrative.
        combined = ((completed.stdout or "") + (completed.stderr or "")).strip()
        return ExecResult(
            # The entire definition of success in this project: exit code 0. Note that
            # unittest exits 5 when it discovers no tests at all, which is correctly NOT a
            # pass — a task whose tests cannot be found must never report SOLVED.
    ...
    def _truncate(self, text: str) -> str:
        """Keep the head of the output, where unittest puts the first failure."""
