...
What it does NOT do, and this is the part worth the workshop's time:
  ...
  - The loop guard's POLICY, though its plumbing is now borrowed. `create_react_agent` wires
    `post_model_hook` by diffing the answered `tool_call_id`s against the calls the model made
    and dispatching only what is left, so answering a call is what refuses it. `guard_node`
    plus `route_after_guard` below is that router, reproduced — the hook is a
    `create_react_agent` argument while `Send` is public — and there is no synthetic AIMessage
    anywhere in this file as a result. What no seam supplies is the claim that an identical
    call means the model is stuck. Two costs came with the shape: the verdict fold moved to
    `fold_node` to stay a single writer, and serialising a turn's calls became the run config's
    job, so `guard_node` refuses to dispatch without `max_concurrency=1`. The guard's own
    counters are `AgentState` fields, scoped to the run because the state is; agent/prebuilt.py
    keeps the same two on a middleware instance and records what that costs.
...
from agentlang.agent.state import AgentState, initial_state
from agentlang.agent.trace import Tracer, TraceEvent, prompt_tokens_of
from agentlang.sandbox.base import ExecResult
from agentlang.tasks.loader import Task
from agentlang.tools.base import WorkspaceChanged
...
def build_graph(
    ...
    def guard_node(state: AgentState, config: RunnableConfig) -> dict[str, Any]:
        ...
        This is the framework's own shape for the job, in a hand-built graph.
        `route_after_guard` works out what still needs running by diffing the calls the model
        made against the `tool_call_id`s already answered — so answering a call here is what
        refuses it, and the surviving subset never has to be restated anywhere.
        `create_react_agent` wires its `post_model_hook` exactly this way. The hook itself is a
        `create_react_agent` argument rather than a `StateGraph` one, but `Send` is public, so
        the shape is ours to use.

        What no framework supplies is the policy: that a call identical to the last one means
        the model learned nothing from the result and re-running it would buy the same output
        for another step.
        ...
        # inside this node; now the dispatch below is the GRAPH's, one task per call, so the
        # only place that can serialise it is the run config. `max_concurrency=1` does throttle
        # Send fan-out — but only from there, out of this file's reach.
        #
        # Which is why it is checked rather than documented and hoped for. Without it, a turn
        # calling write_file and run_tests together can have the tests measure the file as it
        # was BEFORE the write, and `fold_node` would take that stale-but-green ExecResult as
        # the verdict — the precise "believe the tests, not the model" guarantee this project is
        # built on. A raise rather than an assert: `python -O` strips asserts, and a false
        # SOLVED is not the kind of thing to lose to an optimisation flag.
    ...
    def fold_node(state: AgentState) -> dict[str, Any]:
        ...
        This node exists because of the fan-out. With one task per call, several tool tasks
        land in the same superstep, and two writes to one reducer-free key in one step is
        something LangGraph refuses outright. Folding here keeps one writer, which lets
        `tests_passed` stay a plain bool: state.py argues for that on its own merits, because a
        reducer is handed (current, incoming) and cannot tell "the suite went green" from "the
        workspace changed, so the verdict is void".

        It also carries the invariant the tool step has to uphold. Every call the model made
        must get exactly one reply, matched by `tool_call_id`; leave one unanswered and the API
        rejects the NEXT request, a turn away from the cause. Checked here because here is the
        first point where the whole turn is visible — the guard's refusals and the tools'
        answers together.
        """
        messages = state["messages"]
        # THIS turn's replies, and no more: everything appended after the last AIMessage. The
        # fold recognises artifacts by TYPE, and a checkpoint round-trip hands them back as
        # plain dicts — so folding the whole history would quietly stop recognising anything.
        last_ai = next(
            index
    ...
    def route_after_agent(state: AgentState) -> str:
        ...
        Return one of "guard", "nudge", or END.
    ...
    def route_after_guard(state: AgentState) -> str | list[Send]:
        ...
        The diff is scoped to THIS TURN, deliberately stricter than the framework's own version
        of this router, which collects answered ids from the whole message history and so
        assumes `tool_call_id`s are globally unique. Real ones are — ChatOllama synthesises a
        uuid per call — but the ids are the MODEL's to choose, and a model that reused one would
        have its second call silently treated as answered and never run. A turn is the only
        window in which the pairing is guaranteed to mean anything.
...
def run_agent(
    ...
    # turns: a turn is agent + guard + tools + fold, and sometimes a nudge as well. Set generously, because
    ...
    final: AgentState = app.invoke(
        ...
        config={
            ...
            "callbacks": [tracer],
            # Not a performance knob. The tool step fans out one task per call, and
            # these tools are not independent: run_tests measures what write_file just
            # wrote. One worker keeps a turn's calls in the order the model asked for
            # them, which is what `guard_node` refuses to run without.
            "max_concurrency": 1,
