...
What it does NOT do, and this is the part worth the workshop's time:
  ...
  - Either loop guard's POLICY, though the action guard's plumbing is now borrowed.
    ...
    what refuses it. `guard_node` plus `route_after_guard` below is that router, reproduced —
    the hook is a `create_react_agent` argument while `Send` is public — and there is no
    synthetic AIMessage anywhere in this file as a result. Two costs came with the shape: the
    verdict fold moved to `fold_node` to stay a single writer, and serialising a turn's calls
    became the run config's job. What no seam supplies is the claim that an identical call
    means the model is stuck. For the THINKING guard no seam fits at all — `after_model` could
    count idle turns, but the counter would live on the middleware rather than in
    `AgentState.idle_turns`, which is the same problem one step on.
